🦅 TalonGames
Access the web. No restrictions.
Let's be real: the internet was meant to be open, but sometimes firewalls get in the way. I built TalonGames (under the alias Keystone) as a solo project to bridge that gap. It’s a fast, sleek web proxy designed to help anyone bypass censorship and get back to the entertainment and info they actually want to see.
...
The Proxy Layer: Instead of your browser reaching out directly to a blocked site, the request is routed through a proxy scripts (managed in config.js and our sw.js service worker).

Rewriting the Web: The proxy fetches the site’s data and "rewrites" it on the fly. This ensures that links, images, and scripts on the target site continue to work within the proxy environment without "leaking" back to your real IP or getting caught by a filter.
...
👤 The Solo Journey
This project is entirely a solo effort. From the initial index.html to the complex service worker logic, I’ve handled the design, the backend routing, and the constant cat-and-mouse game of bypassing filters.
